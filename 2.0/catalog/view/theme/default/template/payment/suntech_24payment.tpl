<form class="form-horizontal" action="<?php echo $action; ?>" method="POST" id="payment">
  ...
  <input type="hidden" name="ProductName1" value="<?php echo $ProductName1; ?>" />
  <input type="hidden" name="ProductPrice1" value="<?php echo $ProductPrice1; ?>" />
  <input type="hidden" name="ProductQuantity1" value="<?php echo $ProductQuantity1; ?>" />
...
<script>
	// 訂單送出前 , 清空購物車 , 建立訂單
	function saveOrder(){
		$.post( "index.php?route=payment/suntech_24payment/saveOrder", function( data ) {
