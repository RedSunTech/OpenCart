...
class ControllerPaymentSuntechBuysafe extends Controller {
	...
	public function index() {
		...
		$data['OrderInfo'] = '';
		$data['Td'] = $this->session->data['order_id'];
		...
		$data['note1'] = $order_id;
		$data['note2'] = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_buysafe/callback';
		...
		$payment_template .= (strpos(VERSION, '2.2.') !== false) ? '/payment/suntech_buysafe.tpl' : '/template/payment/suntech_buysafe.tpl';
			...
			if ($isSuccess) {
				if (($data['errcode'] == '00') || ($data['errcode'] == '0')) {
				    ...
				    if (!$order_info['order_status_id']) {
				    	$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('suntech_buysafe_order_status_id'), $message, true);
				    } else {
				    	$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('suntech_buysafe_order_status_id'), $message, false);
				    }
