...
class ControllerPaymentSuntech24Payment extends Controller {
	...
	public function index() {
		...
		$data['note1'] = $order_id;
		$data['note2'] = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_24payment/callback';
		...
		$data['ProductName1'] = $this->config->get('suntech_24payment_product_name');
		$data['ProductPrice1'] = $total_amount;
		...
		$payment_template .= (strpos(VERSION, '2.2.') !== false) ? '/payment/suntech_24payment.tpl' : '/template/payment/suntech_24payment.tpl';
		...
		if (isset($data['note2'])) {
			...
			if ($isSuccess) {
				if ($data['EntityATM'] != '') {
			        ...
			        $data['text_response'] = sprintf($this->language->get('text_response'),$data['EntityATM']);
		...
		if (isset($data['note2'])) {
			...
			if ($isSuccess) {
				if ($data['errcode'] == '00') {
				    ...
				    // 更新訂單狀態
				    if (!$order_info['order_status_id']) {
				    	$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('suntech_24payment_order_status_id'), $message, true);
				    } else {
				    	$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('suntech_24payment_order_status_id'), $message, true);
				    }
