...
class ControllerPaymentSuntechChinapay extends Controller {
	...
	public function index() {
		...
		$data['note1'] = $order_id;
		$data['note2'] = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_chinapay/confirm';
			...
			if ($isSuccess) {
				if ($data['errcode'] == '00') {
				    ...
				    if (!$order_info['order_status_id']) {
				    	$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('suntech_chinapay_order_status_id'), $message, true);
				    } else {
				    	$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('suntech_chinapay_order_status_id'), $message, true);
				    }
