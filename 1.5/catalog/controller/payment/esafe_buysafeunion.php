...
class ControllerPaymentEsafeBuysafeUnion extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/esafe_buysafeunion');
        $this->load->model('checkout/order');

        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $itest_mode = $this->config->get('esafe_buysafeunion_test_mode');
        $Term = ""; //分期期數
        $target = "_self";
        ...
        $shtml .="<input type='hidden' name='web' value='" . $this->config->get('esafe_buysafeunion_storeid') . "' />"; //商店代號
        ...
        $shtml .="<input type='hidden' name='Card_Type' value='1' />"; //交易類別 0 信用卡交易、1 銀聯卡交易
        //$shtml .="<input type='hidden' name='Term' value='" . $Term . "' />"; //分期期數
        //$shtml .="<input type='hidden' name='CargoFlag' value='1' />";
        $shtml .="<input type='hidden' name='note1' value='buysafeunion' />";
        $shtml .="<input type='hidden' name='ChkValue' value='" . strtoupper(sha1($this->config->get('esafe_buysafeunion_storeid') . $this->config->get('esafe_buysafeunion_password') . $total_pay . $Term)) . "' />";
        ...
        $this->data['esafe_buysafeunion_description'] = nl2br($this->config->get('esafe_buysafeunion_description_' . $this->config->get('config_language_id')));

        $this->data['continue'] = $this->url->link('payment/esafe_buysafeunion/confirm', '', '');

        if (isset($this->session->data['doubleclick'])) {
            unset($this->session->data['doubleclick']);
        }

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/esafe_buysafeunion.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/esafe_buysafeunion.tpl';
        } else {
            $this->template = 'default/template/payment/esafe_buysafeunion.tpl';
        }
        $this->render();
    }
    ...
    public function confirm() {
        ...
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('esafe_buysafeunion_order_status_id'), $order_info['comment']);
        //$this->cart->clear();
    }
