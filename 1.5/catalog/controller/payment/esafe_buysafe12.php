...
class ControllerPaymentEsafeBuysafe12 extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/esafe_buysafe12');
        $this->load->model('checkout/order');

        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $itest_mode = $this->config->get('esafe_buysafe12_test_mode');
        $Term = "12"; //分期期數
        $target = "_self";
        ...
        $shtml .="<input type='hidden' name='web' value='" . $this->config->get('esafe_buysafe12_storeid') . "' />"; //商店代號
        ...
        $shtml .="<input type='hidden' name='note1' value='buysafe12' />";
        $shtml .="<input type='hidden' name='ChkValue' value='" . strtoupper(sha1($this->config->get('esafe_buysafe12_storeid') . $this->config->get('esafe_buysafe12_password') . $total_pay . $Term)) . "' />";
        ...
        $this->data['esafe_buysafe12_description'] = nl2br($this->config->get('esafe_buysafe12_description_' . $this->config->get('config_language_id')));

        $this->data['continue'] = $this->url->link('payment/esafe_buysafe12/confirm', '', '');

        if (isset($this->session->data['doubleclick'])) {
            unset($this->session->data['doubleclick']);
        }

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/esafe_buysafe12.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/esafe_buysafe12.tpl';
        } else {
            $this->template = 'default/template/payment/esafe_buysafe12.tpl';
        }
        $this->render();
    }
    ...
    public function confirm() {
        ...
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('esafe_buysafe12_order_status_id'), $order_info['comment']);
        //$this->cart->clear();
    }
