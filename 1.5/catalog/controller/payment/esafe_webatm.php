...
class ControllerPaymentEsafeWebATM extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/esafe_webatm');
        $this->load->model('checkout/order');

        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $itest_mode = $this->config->get('esafe_webatm_test_mode');
        $Term = ""; //分期期數
        $target = "_self";
        ...
        $shtml .="<input type='hidden' name='note1' value='webatm' />";
...
//$shtml .= '<script type="text/javascript">document.myform.submit();</script>';
        ...
        $this->data['esafe_webatm_description'] = nl2br($this->config->get('esafe_webatm_description_' . $this->config->get('config_language_id')));

        $this->data['continue'] = $this->url->link('payment/esafe_webatm/confirm', '', '');

        if (isset($this->session->data['doubleclick'])) {
            unset($this->session->data['doubleclick']);
        }

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/esafe_webatm.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/esafe_webatm.tpl';
        } else {
            $this->template = 'default/template/payment/esafe_webatm.tpl';
        }
        $this->render();
    }
    ...
    public function confirm() {
        ...
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('esafe_webatm_order_status_id'), $order_info['comment']);
        //$this->cart->clear();
    }
