...
class ControllerPaymentEsafe24PayShip extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/esafe_24payship');
        $this->load->model('checkout/order');

        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $itest_mode = $this->config->get('esafe_24payship_test_mode');
        $target = "_self";
        ...
        $shtml .="<input type='hidden' name='web' value='" . $this->config->get('esafe_24payship_storeid') . "' />"; //商店代號
        ...
        $NewDate = Date('Ymd', strtotime("+" . $this->config->get('esafe_24payship_maxdays') . " days"));
        ...
        $shtml .="<input type='hidden' name='ProductName1' value='" . $this->config->get('esafe_24payship_productname') . "' />"; //產品名稱
        ...
        $shtml .="<input type='hidden' name='CargoFlag' value='1' />"; //搭配物流 

        $shtml .="<input type='hidden' name='ChkValue' value='" . strtoupper(sha1($this->config->get('esafe_24payship_storeid') . $this->config->get('esafe_24payship_password') . $total_pay)) . "' />";
        ...
        $this->data['esafe_24payship_description'] = nl2br($this->config->get('esafe_24payship_description_' . $this->config->get('config_language_id')));

        $this->data['continue'] = $this->url->link('payment/esafe_24payship/confirm', '', '');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/esafe_24payship.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/esafe_24payship.tpl';
        } else {
            $this->template = 'default/template/payment/esafe_24payship.tpl';
        }
        $this->render();
    }
    ...
    public function result() {
        $this->language->load('payment/esafe_24payship');
        if ($_POST["ChkValue"] == strtoupper(sha1($_POST["web"] . $this->config->get('esafe_24payship_password') . $_POST['buysafeno'] . $_POST["MN"] . $_POST["EntityATM"]))) {

            $this->load->model('checkout/order');
            $order_info = $this->model_checkout_order->getOrder($_POST['Td']); //取得訂單資訊
            $status = $this->config->get('esafe_24payship_order_status_id');
        ...
        elseif ($_POST["ChkValue"] == strtoupper(sha1($_POST["web"] . $this->config->get('esafe_24payship_password') . $_POST['buysafeno'] . $_POST['StoreType']))) {
            //物流狀態
            $this->load->model('checkout/order');
            $order_info = $this->model_checkout_order->getOrder($_POST['Td']); //取得訂單資訊
            ...
            if (!isset($_POST["SendType"]) || ($_POST["SendType"] == 1)) {
                if ($_POST["StoreType"] == '1010') { //用戶已領
                    $finish_status = $this->config->get('esafe_24payship_order_finish_status_id');
                }
                elseif ($_POST["StoreType"] == '101') { //抵達門市
                    $finish_status = $this->config->get('esafe_24payship_order_shipped_status_id');
                }
                else {
                    $finish_status = $this->config->get('esafe_24payship_order_paid_status_id');
                }
    ...
    public function callback() {
        $this->language->load('payment/esafe_24payship');
        if ($_POST["ChkValue"] == strtoupper(sha1($_POST["web"] . $this->config->get('esafe_24payship_password') . $_POST['buysafeno'] . $_POST['MN'] . $_POST['errcode'] . $_POST['CargoNo']))) {
            ...
            //檢查web,MN是否正確
            if (($_POST["web"]!=$this->config->get('esafe_24payship_storeid')) || (intval($_POST["MN"])!=intval(round($order_info['total']))) || (intval($_POST["MN"])==0)) {
                echo '9999';
                exit();
            }
            ...
            if (!isset($_POST["SendType"]) || ($_POST["SendType"] == 1)) {
                    ...
                    // 商家用訊息
                    $comment = sprintf($this->language->get('text_order_shop'),urldecode($_POST["CargoNo"]),urldecode($_POST["StoreName"]),urldecode($_POST["StoreID"]));
                    $strSQL = "INSERT INTO " . DB_PREFIX . "order_history (order_id, order_status_id, notify, comment, date_added) values ('" . $order_info['order_id'] . "', '" . $status . "', '0', '" . $comment . "', NOW());";
                    $this->db->query($strSQL);
                    ...
                    $comment = $this->language->get('text_errmsg') . (($_POST['errcode']!='') ? sprintf($this->language->get('text_failure_reason_code'),$_POST['errcode']):'');
