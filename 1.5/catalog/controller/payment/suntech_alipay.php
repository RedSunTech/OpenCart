...
class ControllerPaymentSuntechAlipay extends Controller {
	protected function index() {
		$this->language->load('payment/suntech_alipay');
...
//  		$this->data['action'] = 'https://test.esafe.com.tw/Service/Etopm.aspx';
		...
		$account = $this->config->get('suntech_alipay_account');
		$password = $this->config->get('suntech_alipay_password');
		...
		$this->data['note2'] = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_alipay/callback';
		$this->data['ChkValue'] = $ChkValue;

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/suntech_alipay.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/suntech_alipay.tpl';
		} else {
			$this->template = 'default/template/payment/suntech_alipay.tpl';
		}
...
//            }
            ...
            if ($isSuccess) {
                ...
                //檢查送出之hash
                $total_amount = intval($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false));
			    $output = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_alipay/callback';
			...
			if ($isSuccess) {
				if ($data['errcode'] == '00') {
				    ...
				    if (!$order_info['order_status_id']) {
				    	$this->model_checkout_order->confirm($order_id, $this->config->get('suntech_alipay_order_status_id'), $message, true);
				    } else {
				    	$this->model_checkout_order->update($order_id, $this->config->get('suntech_alipay_order_status_id'), $message, false);
				    }
