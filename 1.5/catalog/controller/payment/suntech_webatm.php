...
//  		$this->data['action'] = 'https://test.esafe.com.tw/Service/Etopm.aspx';
		...
		$account = $this->config->get('suntech_webatm_account');
		$password = $this->config->get('suntech_webatm_password');
		...
		$this->data['note2'] = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_webatm/callback';
		$this->data['ChkValue'] = $ChkValue;

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/suntech_webatm.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/suntech_webatm.tpl';
		} else {
			$this->template = 'default/template/payment/suntech_webatm.tpl';
		}
...
//            }
            ...
            if ($isSuccess) {
                ...
                //檢查送出之hash
                $total_amount = intval($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false));
			    $output = strtoupper(sha1($account.$password.$order_id.$total_amount)).',payment/suntech_webatm/callback';
			...
			if ($isSuccess) {
				if ($data['errcode'] == '00') {
				    ...
				    if (!$order_info['order_status_id']) {
				    	$this->model_checkout_order->confirm($order_id, $this->config->get('suntech_webatm_order_status_id'), $message, true);
				    } else {
				    	$this->model_checkout_order->update($order_id, $this->config->get('suntech_webatm_order_status_id'), $message, false);
				    }
