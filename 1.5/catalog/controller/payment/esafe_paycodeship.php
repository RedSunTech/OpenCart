...
class ControllerPaymentEsafePaycodeShip extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/esafe_paycodeship');
        $this->load->model('checkout/order');

        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $itest_mode = $this->config->get('esafe_paycodeship_test_mode');
        $target = "_self";
        ...
        $shtml .="<input type='hidden' name='web' value='" . $this->config->get('esafe_paycodeship_storeid') . "' />"; //商店代號
        ...
        $shtml .="<input type='hidden' name='ChkValue' value='" . strtoupper(sha1($this->config->get('esafe_paycodeship_storeid') . $this->config->get('esafe_paycodeship_password') . $total_pay)) . "' />";
        ...
        $this->data['esafe_paycodeship_description'] = nl2br($this->config->get('esafe_paycodeship_description_' . $this->config->get('config_language_id')));

        $this->data['continue'] = $this->url->link('payment/esafe_paycodeship/confirm', '', '');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/esafe_paycodeship.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/esafe_paycodeship.tpl';
        } else {
            $this->template = 'default/template/payment/esafe_paycodeship.tpl';
        }
        $this->render();
    }
    ...
    public function confirm() {
        ...
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('esafe_paycodeship_order_status_id'), $order_info['comment']);
        //$this->cart->clear();
    }
    ...
    public function result() {
        $this->language->load('payment/esafe_paycodeship');
        if ($_POST["ChkValue"] == strtoupper(sha1($_POST["web"] . $this->config->get('esafe_paycodeship_password') . $_POST['buysafeno'] . $_POST['MN'] . $_POST['paycode']))) {

            $this->load->model('checkout/order');
            $order_info = $this->model_checkout_order->getOrder($_POST['Td']); //取得訂單資訊
            $status = $this->config->get('esafe_paycodeship_order_status_id');
            //$finish_status = $this->config->get('esafe_paycodeship_order_finish_status_id');
        ...
        elseif ($_POST["ChkValue"] == strtoupper(sha1($_POST["web"] . $this->config->get('esafe_paycodeship_password') . $_POST['buysafeno'] . $_POST['StoreType']))) {
            //物流狀態
            $this->load->model('checkout/order');
            $order_info = $this->model_checkout_order->getOrder($_POST['Td']); //取得訂單資訊
            ...
            if (!isset($_POST["SendType"]) || ($_POST["SendType"] == 1)) {
                ...
                elseif ($_POST["StoreType"] == '101') { //抵達門市
                    $finish_status = $this->config->get('esafe_paycodeship_order_shipped_status_id');
                }
                else {
                    $finish_status = $this->config->get('esafe_paycodeship_order_paid_status_id');
                }
    ...
    public function callback() {
        $this->language->load('payment/esafe_paycodeship');
        if ($_POST["ChkValue"] == strtoupper(sha1($_POST["web"] . $this->config->get('esafe_paycodeship_password') . $_POST['buysafeno'] . $_POST['MN'] . $_POST['errcode'] . $_POST['CargoNo']))) {
            ...
            //檢查web,MN是否正確
            if (($_POST["web"]!=$this->config->get('esafe_paycodeship_storeid')) || (intval($_POST["MN"])!=intval(round($order_info['total']))) || (intval($_POST["MN"])==0)) {
                echo '9999';
                exit();
            }
