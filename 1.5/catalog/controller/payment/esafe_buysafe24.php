...
class ControllerPaymentEsafeBuysafe24 extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/esafe_buysafe24');
        $this->load->model('checkout/order');

        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $itest_mode = $this->config->get('esafe_buysafe24_test_mode');
        $Term = "24"; //分期期數
        $target = "_self";
        ...
        $shtml .="<input type='hidden' name='web' value='" . $this->config->get('esafe_buysafe24_storeid') . "' />"; //商店代號
        ...
        $shtml .="<input type='hidden' name='note1' value='buysafe24' />";
        $shtml .="<input type='hidden' name='ChkValue' value='" . strtoupper(sha1($this->config->get('esafe_buysafe24_storeid') . $this->config->get('esafe_buysafe24_password') . $total_pay . $Term)) . "' />";
        ...
        $this->data['esafe_buysafe24_description'] = nl2br($this->config->get('esafe_buysafe24_description_' . $this->config->get('config_language_id')));

        $this->data['continue'] = $this->url->link('payment/esafe_buysafe24/confirm', '', '');

        if (isset($this->session->data['doubleclick'])) {
            unset($this->session->data['doubleclick']);
        }

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/esafe_buysafe24.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/esafe_buysafe24.tpl';
        } else {
            $this->template = 'default/template/payment/esafe_buysafe24.tpl';
        }
        $this->render();
    }
    ...
    public function confirm() {
        ...
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('esafe_buysafe24_order_status_id'), $order_info['comment']);
        //$this->cart->clear();
    }
